...
type: concept
area: runtime
updated: 2026-06-11 (migrate command added)
status: mature
load_bearing: false
references:
...
- **`changjuan ingest [--repo-root PATH]`** — Stage 1: reads the dongzhoulieguozhi JSON from `corpora/dongzhoulieguozhi/json/东周列国志.json` and inserts one row per chapter into `corpus.sqlite`. Exits 1 with a clear message if the corpus file is absent. Internally passes an inline DZL `book_meta` dict to `ingest_book(conn, cfg, book_meta)` — a forward-compatible shim until Task 4 wires the book registry.
