...
- **`changjuan link <pipeline_run_id> [--repo-root PATH]`** — Stage 5: runs the linker for the given `pipeline_run_id`. Walks `candidate_persons` for the run, scores each candidate against the canonical + same-run pool (via `stage5_link.link_run`), and dispatches by threshold: auto-merge writes `match_target_id` + `audit_log` entry; mid-score writes a `merge_candidates` row; low-score leaves no trace. Reports a single summary line with `processed / auto-merged / queued / skipped` counts. See `concepts/pipeline/linking.md` for the full linking picture.
