...
`pipeline.dates.parse_date(original: str) -> DateDict` dispatches to pattern-matching helpers in priority order. The function **never raises** — unrecognized inputs return `inference_kind="unknown"` with `year_bce=None`. Dispatch order:

1. `_try_lu` — `鲁X公N年` / lenient prefixes
2. `_try_zhou` — `周X王N年` for all 13 tabulated Zhou kings (平王 through 敬王)
3. `_try_era` — era-range strings (春秋初/早期/中期/末/晚期, 战国初/早期/中期/末/晚期); returns `uncertainty="range"` with midpoint as `year_bce` and `year_bce_end`
4. `_unknown` — fallback; `year_bce=None`, `uncertainty="point"`
