...
- **`changjuan link <pipeline_run_id> [--repo-root PATH]`** — Stage 5: runs the linker for the given `pipeline_run_id`. Sits between `extract-load` and `load` in the user workflow (`extract → extract-load → link → load → golden-eval`). Walks `candidate_persons` for the run, scores each candidate against the canonical + same-run pool (via `stage5_link.link_run`), and dispatches by threshold: score ≥ `LINKER_AUTO_MERGE_THRESHOLD` writes `match_target_id` + `audit_log` entry; score ≥ `LINKER_QUEUE_THRESHOLD` writes a `merge_candidates` row; below that leaves no trace. Reports a single summary line with `processed / auto-merged / queued / skipped` counts. **Idempotent**: re-running `link` on the same run re-uses already-set `match_target_id` values (the `_denormalize_variants` step is a no-op guard; scoring is deterministic), so repeated calls produce the same result. Threshold values: see `pipeline/config.py`. See `concepts/pipeline/linking.md` for the full linking picture.
