...
Task A2 (state prominence pass) adds one more test:

- `test_add_state_prominence_curated_allowlist` — DB pass test for `add_state_prominence`. Seeds two states (`晋`, `滑`), three persons (two in 晋, one in 滑), and three `deed_importance` rows (per:a=300, per:b=100, per:c=5). Writes an overrides YAML listing only `晋` under the `states:` key. Calls `add_state_prominence(graph, overrides)`. Asserts: sta:晋 → major (on the allow-list) with prominence 400.0 (300+100), sta:滑 → minor (not listed) with prominence 5.0. Verifies the deed-sum aggregation via JOIN on `persons.state_id` and the curated allow-list tier (not rank-based) in one fixture.
