...
- **v2 Ch.6 follow-on** (no new directory) — `SKILL.md` updates only: step 3 swapped to `scripts/read-chapter`, step 6.5 added invoking `scripts/check-extraction` between fill-spans and extract-load, and a "Common gotchas" block was prepended capturing the failure patterns from Ch.6 (paraphrasing inside `quote`, bare `<name>曰` dialog tags forcing fill-spans disambiguation, absolute-vs-relative paragraph indexing). The system-prompt body itself is unchanged. See `concepts/pipeline/extraction.md` for the new pre-flight helpers section.
