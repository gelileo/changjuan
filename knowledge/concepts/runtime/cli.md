...
type: concept
area: runtime
updated: 2026-06-11 (extract-load gains --book-id option for multi-book support; ingest/chunk take --book-id; ingest reads corpus source from book-meta; Plan 3b: load gains capability-gated theme loading + themes=N in echo)
status: mature
load_bearing: false
references:
...
affects:
  ...
  - tests/unit/test_publish_depot.py
---
...
- **`changjuan ingest [--book-id ID] [--repo-root PATH]`** — Stage 1: reads `data/books/<book_id>/book-meta.json` to locate the corpus source (`corpus_dir`/`corpus_json`), applies `CORPUS_SCHEMA`, then calls `ingest_book(conn, cfg, meta)` to insert one row per chapter into `corpus.sqlite`. Default `--book-id`: `dzl`. Exits 1 with a clear message if `book-meta.json` is absent or if the resolved corpus source path does not exist. The inline DZL `_dzl_meta` shim (Task 3 forward-compat placeholder) has been removed; dzl now works via its `book-meta.json` like any other book (Task 5 adds `corpus_dir`/`corpus_json` to dzl's meta; until then `ingest --book-id dzl` will KeyError on those keys — that is intentional and expected).
- **`changjuan chunk [--book-id ID] [--repo-root PATH]`** — Stage 2: splits all unchunked documents into paragraph-aware overlapping chunks and writes them to `corpus.sqlite`. `--book-id` (default `dzl`) selects which book's `corpus.sqlite` to operate on via `_cfg(repo_root, book_id)`.
...
- **`changjuan extract-load --chapter N --extraction-file PATH --prompt-version V [--book-id ID] [--pipeline-run-id ID] [--repo-root PATH]`** — Stage 3: validates a skill-produced extraction YAML file, runs all extraction invariants per record, and writes passing records to `candidate_persons`, `candidate_events`, `candidate_places`, `candidate_groups`, and `candidate_relations`. Auto-generates a `pipeline_run_id` (format: `run:extract-ch<chapter>-<prompt_version>-<timestamp>`) if `--pipeline-run-id` not supplied. Returns per-entity-kind counts and lists up to 10 invariant violations (or count if >10). Invariant failures prevent a record from being written but do not cause the command to exit non-zero. `--book-id` (default `dzl`) selects which book's corpus and canonical databases to use — required for multi-book operation (e.g. `--book-id hlm` for 红楼梦).
- **`changjuan link <pipeline_run_id> [--book-id ID] [--ignore-rejections] [--repo-root PATH]`** — Stage 5: runs the linker for the given `pipeline_run_id`. `--book-id` (default `dzl`) selects the book's `canonical.sqlite` via `_cfg(repo_root, book_id)` — required for multi-book linking (e.g. `--book-id hlm`). Sits between `extract-load` and `load` in the user workflow (`extract → extract-load → link → load → golden-eval`). Walks `candidate_persons` for the run, scores each candidate against the canonical + same-run pool (via `stage5_link.link_run`), and dispatches by threshold: score ≥ `LINKER_AUTO_MERGE_THRESHOLD` writes `match_target_id` + `audit_log` entry; score ≥ `LINKER_QUEUE_THRESHOLD` writes a `merge_candidates` row; below that leaves no trace. Reports a single summary line with `processed / auto-merged / queued / skipped / rejected-filter-skipped` counts. **Phase 6 flag: `--ignore-rejections`** — by default, candidate pairs whose merge was previously rejected by the curator (recorded in the `rejected_merges` table) are filtered out at the queue stage. Pass `--ignore-rejections` to bypass that filter and re-emit those pairs (useful when a curator wants to revisit prior rejections). Appends `(ignore-rejections=ON)` to the summary line when active. **Idempotent**: re-running `link` on the same run re-uses already-set `match_target_id` values (the `_denormalize_variants` step is a no-op guard; scoring is deterministic), so repeated calls produce the same result. Threshold values: see `pipeline/config.py`. See `concepts/pipeline/linking.md` for the full linking picture.
- **`changjuan load <pipeline_run_id> [--book-id ID] [--repo-root PATH]`** — Stage 7: promotes all entity-kind candidates matching the given `pipeline_run_id` into canonical entities with field-level merge semantics (curated-never-overwritten, higher-confidence-wins, Conflict on disagreement). Load order: places + groups first (other entity types reference them via foreign keys), then persons, events, and finally all six relation kinds. After relations, themes loading runs if the book's profile declares the `themes` capability (i.e. `"themes" in PROFILES[profile]["capabilities"]`); for the `history` profile `n_themes = 0`, for `cast` profile `load_candidate_themes` runs. The echo summary line always includes all fields: `loaded: places=N groups=N persons=N events=N relations=N themes=N (run=…)`. `--book-id` (default `dzl`) locates `data/books/<book_id>/book-meta.json`; the `profile` field (default `"history"`) gates themes loading and is passed into `load_candidate_groups` and `load_candidate_relations`.
- **`changjuan export <version> [--book-id ID] [--repo-root PATH]`** — Stage 9: freezes a versioned export bundle at `data/exports/<slug>-export-<version>/` (the bundle dir is named by the book's human-facing `slug` from `book-meta.json`, falling back to `book_id` — so `dzl` with `slug: dongzhoulieguozhi` yields `dongzhoulieguozhi-export-<version>/`), containing `manifest.json`, a `candidate_*`-stripped SQLite snapshot (`graph.sqlite`), and a `texts/` directory of chapter prose files. Loads `data/books/<book_id>/book-meta.json` (default book_id: `dzl`) and passes it as `book_meta` to `export_bundle`; `manifest.json` includes book identity and capability fields from that file. If `book-meta.json` is absent, the command exits 1 with a clean message (`book-meta.json not found for book '<id>': <path>`) rather than a raw `FileNotFoundError` traceback. The export will also fail loud if any cited chunk is missing from `corpus.sqlite`. The command passes `readable_dir=cfg.readable_dir` (`data/books/<book-id>/readable/`) to `export_bundle`; if that directory is absent the `texts/` subdirectory is created but left empty (no error). It also passes `prominence_overrides=cfg.data_dir / "prominence_overrides.yaml"` (the curated promote/demote list for person-prominence tiering — see `concepts/pipeline/export-contract.md`); no new flag, and an absent file falls back to auto-only tiering.
- **`changjuan publish-depot --depot PATH --version V [--book-id ID] [--repo-root PATH]`** — Publishes an export bundle into the changjuan-depot working tree. Resolves the export dir as `data/books/<book_id>/exports/<slug>-export-<version>/` from `Config` (the same path `export` writes to). Reads `manifest.json` from the export dir; exits 1 with a clear message if `manifest.json` is absent (export bundle not found). Calls `pipeline.publish_depot.publish_book` which: copies `graph.sqlite` → `<depot>/books/<book_id>/<book_id>-<version>.sqlite`, computes `bytes`/`sha256`, and upserts `<depot>/catalog.json`. If the manifest carries a `prices` key, `build_entry` copies it into the catalog entry (free books omit it). `build_entry` also copies `group_type` (the collective group kind) so a downloaded book labels its groups tab correctly (列国/世家/…). On success echoes `published <book_id>@<version> → <depot>; catalog has N book(s)`. Does NOT git-commit or git-push — the operator runs those manually. Idempotent: re-publishing the same version overwrites the bundle and replaces the catalog entry. Default `--book-id`: `dzl`. See [`concepts/pipeline/depot.md`](../pipeline/depot.md) for the catalog contract.
- **`changjuan migrate [--book-id ID] [--repo-root PATH]`** — One-time, idempotent migration: runs `pipeline.migrations.migrate_0001_state_to_group.run(conn)` against `data/books/<book_id>/canonical.sqlite`. Brings a DB created under the old `states` schema to the groups-named schema (schema_version 7) without re-extraction. Safe to run multiple times — if `groups` already exists and `states` is absent, the migration is a no-op. Default `--book-id`: `dzl`. On success echoes `migrated <path> to groups schema`.

### Sampling QA verbs (Phase 2)

- **`changjuan qa-sample <pipeline_run_id> [--repo-root PATH]`** — Emits the deterministic 5% sample of scalar facts for the given `pipeline_run_id` as YAML on stdout. The `changjuan-verify-sample` skill consumes this output. The verb first checks `candidate_facts` (stage 3 per-field justifications); if that table is empty for the run, it falls back to enumerating scalar fields directly from `candidate_persons`, `candidate_events`, `candidate_places`, and `candidate_groups` (any non-null scalar field becomes one fact). Sampling is bounded by `config.QA_SAMPLE_FLOOR` (30) and `config.QA_SAMPLE_CEILING` (250) and is fully deterministic — same `pipeline_run_id` always produces the same sample.
...
- **`changjuan golden-eval --chapter N [--pipeline-run-id ID] [--repo-root PATH]`** — Loads the golden YAML set for chapter N from `tests/golden/ch{N:02d}/`, queries `pipeline_runs` for the most recent `stage='extract-load'` run whose `scope_json.chapter = N`, fetches all candidate rows (persons, events, places, groups, and all five candidate relation tables), runs `tests/golden/precision_recall.compute_pr`, and prints per-entity-type P/R lines with ✓/✗ vs `GOLDEN_PR_THRESHOLDS`. Exits non-zero if any kind falls below threshold. `--pipeline-run-id` overrides the auto-lookup.

  Candidate-to-dict mapping:
  - **persons** — `id` (chunk-local suffix, e.g. `p1`), `canonical_name`, `group_id`, `social_category`, `variants=[]` (candidate_person_variants is Phase-3 only).
...
- **`changjuan backfill-narrative-dates [--repo-root PATH] [--actor LABEL]`** — Bulk auto-resolution complementing the manual verb above. Walks **all** canonical events in narrative order (chapter, then paragraph, parsed from each event's `chk:dzl:<ch>:<para>` citations) and fills `year_bce` for any still-undated `relative_to_prior_event` event from the nearest **prior** dated event — what "relative to prior event" means, since the novel narrates chronologically. Catches cross-chunk relatives that per-chunk resolution missed (e.g. the 干将 sword event "其后…" → 514, anchored to the adjacent 铸剑). Calls `pipeline.dates.backfill_narrative_neighbor_dates`; the filled date keeps `inference_kind='relative_to_prior_event'` but records the `relative_anchor_event_id` used and a `narrative_inferred: true` flag (honest low-trust provenance). `era_only` flashbacks (e.g. 秦文公之时) and events without a `chk:` citation are deliberately left undated. Writes one `audit_log` row per change (`change_kind = 'set'` — the only audit kinds allowed by the schema CHECK; the inferred year + anchor live in `after_json`), default `actor = system:narrative-backfill`, and echoes the count + each `event_id: year_bce (anchor)`.
