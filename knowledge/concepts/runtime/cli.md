...
type: concept
area: runtime
updated: 2026-06-11 (ingest/chunk take --book-id; ingest reads corpus source from book-meta)
status: mature
load_bearing: false
references:
...
- **`changjuan load <pipeline_run_id> [--book-id ID] [--repo-root PATH]`** — Stage 7: promotes all five entity-kind candidates matching the given `pipeline_run_id` into canonical entities with field-level merge semantics (curated-never-overwritten, higher-confidence-wins, Conflict on disagreement). Load order: places + groups first (other entity types reference them via foreign keys), then persons, events, and finally all six relation kinds. Returns counts for each entity kind loaded. `--book-id` (default `dzl`) is used to locate `data/books/<book_id>/book-meta.json`; the `profile` field from that file (default `"history"`) is passed into both `load_candidate_groups` (to stamp `groups.group_type` from the profile's `default_group_type`) and `load_candidate_relations` (to validate relation `kind` values).
