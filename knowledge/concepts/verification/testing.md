...
area: verification
updated: 2026-06-11
note: "2026-06-11 Plan 3a complete: test_hlm_ingest_120_chapters added (n=120, id hlm:1, corpus honglou); dzl regression test_dzl_ingest_unchanged; 2026-06-11 groups type/group_type split: test_canonical_schema_groups.py, test_state_to_group_migration.py, test_extract_output_groups.py, test_stage7_load_states.py, test_cli.py updated; fixture ch01-extraction-v1.yaml updated (group_type:→type:); 2026-06-11 candidate migration gap-fill: test_state_to_group_migration OLD_SCHEMA + assertions extended; test_dzl_export_parity skips when already migrated; 2026-06-11 Plan 3b Task 1: test_canonical_schema_themes.py (3 tests — tables exist, entity_citations theme kind, candidate_themes columns); 2026-06-11 Plan 3b Task 2: test_extract_output_themes.py (2 tests — optional themes array, theme item shape); 2026-06-11 Plan 3b Task 3: test_load_themes.py (3 tests — creates theme + resolves occurrences, idempotent name match, profile capability assertion); 2026-06-12 test_profile.py gains test_cast_vocab_includes_native_hlm_kinds (consort/subject/neighbor/colleague/patron present; modern labels absent); 2026-06-12 test_extract_load.py gains test_loads_themes_and_person_relation_kind_detail (regression: load_extraction writes candidate_themes from the themes array; person_relation kind comes from kind_detail — both were silent-drop bugs); 2026-06-12 test_extract_load.py gains test_malformed_relation_skipped_not_fatal (a relation missing a required endpoint field is skipped + logged, not a fatal KeyError); 2026-06-13 test_load_groups.py gains test_clan_groups_merge_on_surname_stem (贾府/贾家/贾 → one 贾 node + _clan_stem unit checks)"
status: mature
load_bearing: false
references:
...
`tests/unit/test_stage9_export.py` exercises `pipeline.stage9_export.export_bundle`. Task 21 adds two baseline tests: `test_export_creates_manifest_and_sqlite` (seeds one Person, exports, asserts `manifest.json` and `graph.sqlite` exist, asserts `changjuan.sqlite` does not exist, manifest `version`, `schema_version==2`, and `counts["persons"]==1`) and `test_export_snapshot_is_readable_sqlite` (asserts the snapshot opens as SQLite and contains the `persons` table). Task 22 adds `test_export_strips_all_candidate_tables` (seeds a `candidate_persons` row, exports, asserts no `candidate_*` tables in snapshot — documents the fail-loud stripping contract) and `test_export_strips_llm_cache` (asserts `llm_cache` is absent from snapshot — documents the implementation-detail exclusion). Task 23 adds `test_export_roundtrip_preserves_canonical_data` (seeds 2 Persons with different provenances + 1 Event, exports, opens snapshot with a fresh `sqlite3.connect` handle — no pipeline helpers — checks row ids and provenance values, and confirms manifest `counts["persons"]==2` and `counts["events"]==1` agree with snapshot reality). Export bundle v1 Task 3 (code-review fix) rewrites `test_manifest_includes_book_identity_and_capabilities` to use the shared `_empty_corpus(tmp_path)` helper for the corpus and pass `_MINIMAL_BOOK_META` directly to `export_bundle` — no inline corpus creation or local `meta` variable. Asserts all six identity keys in the manifest equal the corresponding value in `_MINIMAL_BOOK_META`: `book_id`, `title`, `author`, `edition`, `capabilities`, and `cover` (which is `None` → JSON `null`, exercising the optional-null branch). `_MINIMAL_BOOK_META` contains all six keys including `"cover": None` and `"edition": "明刊本"`. Export-bundle-v1 Task 6 adds `test_export_copies_readable_texts`: creates two `ch*.md` files in a `tmp_path / "readable"` directory, exports with `readable_dir=readable`, and asserts `out / "texts" / "ch01.md"` contains the expected text and `ch02.md` is present. This confirms the texts-copy pass operates correctly when `readable_dir.is_dir()` is true. All seven tests pass `readable_dir=tmp_path / "readable"` (either pointing to the populated dir in the new test, or to a non-existent path in the other six — the `is_dir()` guard tolerates a missing dir). All snapshot file references use `graph.sqlite` (v2 layout, `SCHEMA_VERSION=2`). Tasks A3+A4 (schema_version 4) update the `schema_version==3` assertion in `test_export_creates_manifest_and_sqlite` to `schema_version==4` (later bumped to 7 by State→Group rename + manifest_reader_capabilities) — this is a legitimate required assertion change, not a test weakening; the `export_bundle` call in that test exercises the two new enrichment passes (`add_event_prominence`, `add_state_prominence`) and the snapshot now contains `events.prominence(_tier)` + `states.prominence(_tier)`. All snapshot file references use `graph.sqlite` (v2 layout, `SCHEMA_VERSION=4`). All tests supply an `_empty_corpus` helper (chunks-only SQLite, no `documents` table) as the required `corpus_db` argument — `_source_editions` guards against the absent table and returns `{}`. Export-bundle-v1 Task 7 adds `test_snapshot_includes_uncheckpointed_wal_data`, the regression guard for the WAL-safe snapshot fix: it opens a raw `sqlite3` writer on `src` with `journal_mode=WAL` and `wal_autocheckpoint=0`, applies the canonical schema, INSERTs and commits a sentinel Person (`per:wal`), and **keeps that writer connection open across the `export_bundle` call** (closing it could trigger a checkpoint that merges the WAL into the main file and masks the bug). It then asserts the sentinel survives in `graph.sqlite`. This test fails on the old `shutil.copyfile` snapshot (which copies only the main file, dropping un-checkpointed WAL frames) and passes with `VACUUM INTO` (which snapshots the full main+WAL view). A minor-cleanup pass (review notes) adds `assert (out / "texts").is_dir()` to `test_export_creates_manifest_and_sqlite`, confirming that `texts/` is created even when `readable_dir` is absent — the negative-path contract for the `is_dir()` guard. The texts glob is `ch[0-9]*.md` (not `ch*.md`) to exclude non-chapter files like `changelog.md`; `ch01.md`/`ch02.md` in the `test_export_copies_readable_texts` fixture still match.

`test_stage9_export.py` also gains two `group_type` manifest tests (2026-06-14): `test_manifest_group_type_defaults_to_state_for_history` (`_MINIMAL_BOOK_META` has no `profile` → manifest `group_type == "state"`) and `test_manifest_group_type_is_clan_for_cast_profile` (meta with `profile: "cast"` → `group_type == "clan"`). These pin the per-genre groups-tab label contract the reader consumes.

## Export enrichment tests

`tests/unit/test_export_enrich.py` exercises `pipeline.export_enrich`. Four tests:

- `test_build_citations_table_denormalizes_cited_chunks` — seeds `entity_citations` with two `chk:` chunk ids (one duplicate row), one `run:` provenance id (`run:extract-chX-vY` on an `event_participant` edge), and a `chunks` table in a separate corpus DB with three rows (two cited, one uncited); asserts the resulting `citations` table in `graph.sqlite` has exactly the two `chk:` rows with correct text, and that the `run:` id is excluded (`assert "run:extract-chX-vY" not in rows`). Verifies duplicate `citation_id` values are deduplicated (DISTINCT query) and that provenance ids are silently ignored — not resolved or raised.
- `test_build_citations_table_raises_when_chunk_missing` — seeds `entity_citations` with the same fixture (two `chk:` ids + one `run:` id) but supplies an empty `chunks` table; asserts `ValueError` is raised with a message matching "2 cited chunk" — the fail-loud contract for missing `chk:` passages. The `run:` id is excluded by the `WHERE citation_id LIKE 'chk:%'` filter and does not inflate the missing count.
...
Task A2 (group prominence pass) adds one more test:

- `test_add_state_prominence_curated_allowlist` — DB pass test for `add_group_prominence` (imported as `add_state_prominence` in the test for historical symmetry with the test name). Seeds two groups (`晋`, `滑`), three persons (two in 晋, one in 滑), and three `deed_importance` rows (per:a=300, per:b=100, per:c=5). Writes an overrides YAML listing only `晋` under the `states:` key (curated data key, left as-is after the rename). Calls `add_group_prominence(graph, overrides)`. Asserts: sta:晋 → major (on the allow-list) with prominence 400.0 (300+100), sta:滑 → minor (not listed) with prominence 5.0. Verifies the deed-sum aggregation via JOIN on `persons.group_id` and the curated allow-list tier (not rank-based) in one fixture.

schema_version 5 (narrative_seq) adds one more test and bumps one assertion:
...
- `test_backfill_narrative_dates_fills_and_audits` — seeds `entity_citations` so `evt:anchor` (771) precedes `evt:rel` in chapter 7 by paragraph, invokes `backfill-narrative-dates`, and asserts `evt:rel.year_bce == 771` with `relative_anchor_event_id=evt:anchor` + `narrative_inferred=True`, plus exactly one `audit_log` row with `change_kind='set'`. This is the **regression guard for the audit `change_kind` CHECK constraint**: the verb originally used an invalid kind (`'narrative_backfill'`) and raised `IntegrityError` at runtime — the pure-function unit test didn't touch the real schema, this CLI test does.
...
Four new tests lock the "promotion waiver" branch in `person_match_score` (added after Ch.6-10 batching surfaced 4 of 5 queued candidates fitting the same shape — strong variant + same state + diff social_category, all real role-evolution cases).

- `tests/unit/test_scoring.py::test_strong_variant_same_state_diff_social_no_penalty` — positive case: two records with the same canonical_name 公子佗, same state sta:chen, `social_category` noble vs royalty (the 篡立 promotion). Asserts the score reaches 0.70 (vs 0.60 pre-fix), exactly at the auto-merge floor.
- `tests/unit/test_scoring.py::test_diff_social_still_penalized_when_state_differs` — cross-state regression: two records named 公子元 in sta:zheng vs sta:qi (the actual Ch.7 vs Ch.8 collision); asserts the score clamps to 0.0 (waiver does not apply when state differs).
- `tests/unit/test_scoring.py::test_diff_social_still_penalized_with_partial_variant` — partial-match regression: two records with overlapping variants but distinct canonicals (variant_overlap=partial); asserts the −0.10 penalty still fires, score = 0.30.
- `tests/unit/test_linker.py::test_promotion_pattern_auto_merges` — end-to-end via `link_run`: seed canonical 公子佗 (noble) + same-name candidate (royalty) in sta:chen; asserts `auto_merges == 1` and `queued == 0`.
