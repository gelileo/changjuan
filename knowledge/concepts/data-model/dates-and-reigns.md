...
`pipeline.dates.parse_date(original: str) -> DateDict` dispatches to pattern-matching helpers in priority order. The Lu parser (`_try_lu`) uses `_LU_PATTERN` which accepts both the full `鲁X公N年` form and lenient prefixes (`鲁X年` / `X公N年`). The Zhou parser (`_try_zhou`) matches `周X王N年` for all 13 Zhou kings tabulated in the reign table (平王 through 敬王). The Chinese numeral parser (`_cn_to_int`) covers 元 and compound forms up to ~60.
