...
```
/changjuan-extract chapter:N
```

`N` is an integer (1–108). The output will be written to
`data/extractions/ch{N:02d}/extract-v1.yaml`.
...
- `.claude/skills/changjuan-extract/system-prompt.md` — extraction rules (Chinese)
- `.claude/skills/changjuan-extract/extraction-schema.yaml` — canonical field/type reference
- `.claude/skills/changjuan-extract/examples/ch01-excerpt.md` — fully worked example
...
# write the YAML to:
# data/extractions/ch$(printf "%02d" $CHAPTER)/extract-v1.yaml
```

### 6. Fill span offsets

```bash
./scripts/fill-spans data/extractions/ch$(printf "%02d" $CHAPTER)/extract-v1.yaml
```
...
uv run changjuan extract-load \
  --chapter $CHAPTER \
  --extraction-file data/extractions/ch$(printf "%02d" $CHAPTER)/extract-v1.yaml \
  --prompt-version v1
```
