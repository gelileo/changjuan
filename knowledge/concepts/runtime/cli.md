...
- **`changjuan export <version> [--repo-root PATH]`** — Stage 9: freezes a versioned export bundle at `data/exports/changjuan-export-<version>/`, containing `manifest.json` and a `candidate_*`-stripped SQLite snapshot. Internally passes `corpus_db=cfg.corpus_db` to `export_bundle`; the export will fail loud if any cited chunk is absent from `corpus.sqlite`.
